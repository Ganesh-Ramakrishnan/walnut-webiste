...
import { CONTACT_EMAIL, DISCORD_URL, INSTAGRAM_URL, LINKEDIN_URL, MAILTO_URL, MAPS_URL } from "@/lib/links";
...
/* Every card carries the same four rows — icon, one-line title, one-line
   action, then a short line of support copy — so the five line up. */
const contactMethods = [
  ...
  },
  {
    ...
    href: LINKEDIN_URL,
    sub: "Stay updated with our latest news",
  },
  {
    icon: (
  ...
  },
  {
    ...
    detail: "Open in Maps",
    href: MAPS_URL,
    sub: "Bhartiya City, Bengaluru 560064",
  },
];
        ...
        <section className="cm-section">
          ...
          <div className="cm-grid">
            {contactMethods.map((m) => {
              ...
              if (!m.href) {
                return <div key={m.title} className="cm-card">{inner}</div>;
              }

              // The whole card is the target. mailto: stays in this tab so the
              // mail client takes over; web destinations open in a new one.
              const newTab = m.href.startsWith("http");
