...
export default function EditBlogPost() {
  ...
  const [form, setForm] = useState({ title: "", slug: "", excerpt: "", content: "", author: "", authorTitle: "", authorLinkedin: "", image: "", category: "", tags: "", date: "", seoTitle: "", metaDescription: "" });
  ...
  useEffect(() => {
    fetch(`/api/blogs/${slug}`)
      ...
      .then((post) => {
        setForm({ title: post.title, slug: post.slug, excerpt: post.excerpt, content: post.content, author: post.author, authorTitle: post.authorTitle || "", authorLinkedin: post.authorLinkedin || "", image: post.image || "", category: post.category, tags: Array.isArray(post.tags) ? post.tags.join(", ") : post.tags, date: post.date, seoTitle: post.seoTitle || "", metaDescription: post.metaDescription || "" });
        if (post.relatedLinks && Array.isArray(post.relatedLinks)) setRelatedLinks(post.relatedLinks);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/blogs/${slug}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, tags: form.tags.split(",").map((t) => t.trim()).filter(Boolean), faqs: faqs.filter((f) => f.question && f.answer), relatedLinks: relatedLinks.filter((l) => l.label && l.href) }) });
      if (res.ok) router.push("/dashboard/blogs");
      else { const data = await res.json(); alert(data.error || "Failed to update post"); }
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try { const res = await fetch(`/api/blogs/${slug}`, { method: "DELETE" }); if (res.ok) router.push("/dashboard/blogs"); else alert("Failed to delete post"); }
  ...
  if (loading) return <div className="d-empty">Loading...</div>;
  if (notFound) return (
    <div className="d-empty-state">
      <h2>Post not found</h2>
      <p>The blog post &quot;{slug}&quot; doesn&apos;t exist.</p>
      <a href="/dashboard/blogs" className="d-btn d-btn--primary">Back to Posts</a>
    </div>
  );
  ...
  return (
    <div>
      <div className="d-page-head">
        <div>
          <h1 className="d-page-title">Edit Post</h1>
          <p className="d-page-sub">Editing: {form.title}</p>
        </div>
        <div className="d-actions">
          <button type="button" className="d-btn d-btn--ghost" onClick={() => setPreview(!preview)}>{preview ? "Edit" : "Preview"}</button>
          <button type="button" className="d-btn d-btn--danger" onClick={() => setShowDeleteModal(true)}>Delete</button>
        </div>
      </div>
      ...
      {preview ? (
        <div className="d-card" style={{ padding: 32 }}>
          ...
          <div className="d-preview-head">
            ...
            <p className="d-muted">{form.author} &middot; {form.date}</p>
        ...
        <div className="d-card" style={{ padding: 28 }}>
          <form onSubmit={handleSubmit} className="d-form">
            ...
            <div className="d-row">
              <div className="d-field"><label htmlFor="title">Title *</label><input id="title" type="text" value={form.title} onChange={(e) => updateField("title", e.target.value)} required /></div>
              <div className="d-field"><label htmlFor="slug">Slug</label><input id="slug" type="text" value={form.slug} onChange={(e) => updateField("slug", e.target.value)} /></div>
            </div>
            ...
            <div className="d-form-foot">
              <button type="submit" className="d-btn d-btn--primary" disabled={saving}>{saving ? "Updating..." : "Update Post"}</button>
      ...
      {showDeleteModal && <DeleteModal title={form.title} onConfirm={handleDelete} onCancel={() => setShowDeleteModal(false)} deleting={deleting} />}
    </div>
  );
}
